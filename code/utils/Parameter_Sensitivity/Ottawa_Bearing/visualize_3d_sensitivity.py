    ...
    for file in os.listdir(folder_path):
        if file.endswith(".log"):
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                # 直接通过 Mean_Accbyloss 分割每一组实验
                # 每组实验通常以一组参数开始，以 Mean_Accbyloss 结束
                # 我们寻找所有 Mean_Accbyloss 出现的位置，并向前看
                acc_positions = [m.end() for m in re.finditer(r"Mean_Accbyloss:", content)]
...
def plot_3d_sensitivity(df, save_dir):
    if df.empty:
        print("未提取到有效数据，请检查日志格式是否包含 Mean_Accbyloss 等关键字。")
        return
...
if __name__ == "__main__":
    ...
    if not df_results.empty:
        ...
        print("dataset_name:, target_id:, threshold:, physics_thresh:, Mean_Accbyloss:")
